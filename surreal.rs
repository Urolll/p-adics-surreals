...
struct Surreal {
	l: Option<Vec<i32>>,
	r: Option<Vec<i32>>,
	// such that a Surreal X = { l | r }
}

fn construct(num: &str) -> Surreal {
	// takes a String of the form "{ L | R }" by parsing
	let trimmed = num.trim_matches(|c| c == '{' || c == '}' || c == ' ').trim();
	...
	let lhs = if parts.len() > 0 && !parts[0].trim().is_empty() { Some(result(parts[0])) } else { None };
	let rhs = if parts.len() > 1 && !parts[1].trim().is_empty() { Some(result(parts[1])) } else { None };
